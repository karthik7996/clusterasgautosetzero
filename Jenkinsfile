pipeline {
    ...
    environment {
        ...
        STACK_NAME = 'ClusterAsgUpdatestack'
        TEMPLATE_FILE = 'autoscalling.yaml'
        PROJECT_NAME = 'ClusterAsgUpdatestack'
    ...
    stages {
       stage('checkout'){
            steps{
                git credentialsId: 'MY_BB_CRED', url: 'https://SreekantReddy@bitbucket.org/cloudzenix/clusterasgautosetzero.git'
            }
       }
        stage('Build') {
            steps {
        ...
        stage('Deploy to Cloudzenix'){
            when {
                expression { "${params.Env}" == 'Cloudzenix&Devsbx1' }
                 }
            steps {
                echo "${BRANCH_NAME}"
                script {
